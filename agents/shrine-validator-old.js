/**
 * Shrine Component Validator Agent
 * Event-driven validation of bundle components
 * Checks availability at bundle creation, pre-listing, and during active listings
 */
...
async function checkComponentAvailability(component) {
  ...
  if (last_checked && Date.now() - new Date(last_checked).getTime() < 3600000) {
    return { status: 'skip', reason: 'Checked within last hour' };
  }
  ...
  try {
    ...
    const response = await axios.get(item_url, {
      timeout: 10000,
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; ShrineValidator/1.0)'
      }
    });
...
async function runValidation(db, options = {}) {
  ...
  // Check for new signals from lister (bid/buy events)
...
async function standaloneMode() {
  console.log('🔄 VALIDATOR: Running standalone mode');
...
if (require.main === module) {
  ...
  if (mode === 'standalone') {
    ...
    console.error('Usage: node shrine-validator.js [standalone|validate <bundle_id>]');
    process.exit(1);
  }
}
